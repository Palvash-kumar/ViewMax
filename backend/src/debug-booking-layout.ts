...
async function run() {
  ...
  const ShowtimeSchema = new mongoose.Schema(
    {},
    { strict: false, collection: 'showtimes' },
  );
  ...
  layouts.forEach((l) => {
    console.log(
      `- ID: ${l._id} | Name: ${l.get('layoutName')} | Status: ${l.get('status')}`,
    );
    console.log(
      '  ScreenConfig:',
      JSON.stringify(l.get('screenConfig'), null, 2),
    );
    console.log(
      '  Generated3DData.screen:',
      JSON.stringify(l.get('generated3DData')?.screen, null, 2),
    );
  });
