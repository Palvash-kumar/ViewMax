...
export class AnalyticsService {
  ...
  async getPlatformStats() {
    ...
    const [totalUsers, newUsers7d, totalBookings, confirmedBookings, revenueData, cancelledBookings] =
      await Promise.all([
        this.userModel.countDocuments().exec(),
        this.userModel.countDocuments({ createdAt: { $gte: sevenDaysAgo } }).exec(),
        this.bookingModel.countDocuments().exec(),
        this.bookingModel.countDocuments({ bookingStatus: { $in: [BookingStatus.CONFIRMED, BookingStatus.CHECKED_IN] } }).exec(),
        this.bookingModel.aggregate([
          { $match: { bookingStatus: { $in: [BookingStatus.CONFIRMED, BookingStatus.CHECKED_IN] } } },
          { $group: { _id: null, totalRevenue: { $sum: '$totalAmount' }, count: { $sum: 1 } } },
        ]).exec(),
        this.bookingModel.countDocuments({ bookingStatus: BookingStatus.CANCELLED }).exec(),
      ]);
  ...
  async getTopTheatres(limit = 5) {
    return this.bookingModel.aggregate([
      ...
      { $lookup: { from: 'showtimes', localField: 'showtimeId', foreignField: '_id', as: 'showtime' } },
      { $unwind: '$showtime' },
      { $lookup: { from: 'theatres', localField: 'showtime.theatreId', foreignField: '_id', as: 'theatre' } },
      { $unwind: '$theatre' },
      {
        $group: {
          _id: '$theatre._id',
